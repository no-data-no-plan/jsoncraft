...
faq:
  - q: "¿Por qué JSON no tiene comentarios?"
    a: "JSON se diseñó como formato de intercambio máquina-a-máquina, no como formato de config editado por humanos. Douglas Crockford, que especificó JSON, quitó deliberadamente los comentarios porque los parsers estaban implementando directivas no portables en ellos, rompiendo la interoperabilidad. La omisión es feature para tráfico de cable y propiedad hostil para configuración. Si te descubres queriendo comentarios en un `.json` de config, esa es la señal para convertirlo a YAML o TOML — ambos soportan comentarios con `#` nativamente y ambos tienen tooling maduro."
  - q: "¿Cuándo es peligroso YAML de verdad?"
    a: "YAML tiene dos footguns reales de seguridad. La deserialización basada en tags permite que un documento malicioso instancie clases arbitrarias durante el parseo vía tags como `!!python/object:...`, `!!java...` o `!!ruby/object:...` — usa siempre `yaml.safe_load()` en Python, `SafeConstructor` en snakeyaml o `YAML(typ='safe')` en ruamel sobre input no confiable. La otra trampa es el problema de Noruega: `NO` sin comillas se parseaba como booleano `false` en YAML 1.1 y muchos parsers siguen esa semántica. Entrecomilla códigos de país y tokens string-ish similares defensivamente."
  - q: "¿Por qué TOML funciona para Cargo pero no para configs profundos?"
    a: "TOML se diseñó para configuración plana o con anidamiento moderado. Su sintaxis de tablas y claves con puntos es limpia para estructuras superficiales — `Cargo.toml` y `pyproject.toml` son mayormente listas de paquetes, feature flags y opciones de build. Representar un objeto anidado a 5 niveles en TOML es posible pero se lee mal, y TOML no tiene mecanismo nativo de referencia o herencia, así que acabas repitiéndote. Si tu config es profunda, con referencias cruzadas o multi-environment, YAML lo maneja más naturalmente gracias a anchors y aliases."
  - q: "¿Es YAML un superset de JSON?"
    a: "Desde YAML 1.2, técnicamente sí — todo documento JSON válido parsea como YAML. Fue un objetivo explícito, así que puedes pegar un blob JSON en un archivo YAML y el parser lo acepta. En la práctica, 'superset' oculta que YAML añade whitespace significativo, múltiples documentos, anchors, múltiples estilos de escalar y tipado implícito encima. Así que aunque JSON-en-YAML funciona, las features extra de YAML hacen que un round trip vía JSON pierda comentarios y anchors. Convierte libremente en un sentido (YAML → JSON), pero trata el inverso como con pérdidas."
  - q: "¿Puedo convertir con seguridad entre JSON, YAML y TOML?"
    a: "Los datos estructurales hacen round-trip limpio; la metadata no. YAML → JSON pierde comentarios y expande anchors inline, lo que infla el archivo. JSON → YAML es lossless en estructura pero tu emisor puede usar block style o flow style por defecto — revisa la salida. TOML ↔ JSON es el round trip más limpio porque TOML tiene tipos explícitos que mapean 1:1 a primitivos JSON, con un pero: los tipos date nativos de TOML se convierten en strings planos en JSON. Usa los conversores en navegador de JSONCraft — los datos no salen de la página."
---
